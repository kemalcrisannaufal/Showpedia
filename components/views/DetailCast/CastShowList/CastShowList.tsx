...
const CastShowList = (props: Proptypes) => {
  ...
  const { maxIdxShow, handleShowAllOrLess } = useCastShowList();
                ...
                <ul className="mt-2 pl-5 lg:pl-10">
                  {castcredit.map((item) => (
                    <li key={item._embedded?.episode?.id} className="mb-1.5">
                      <Link
                        href={`/shows/${getIdShowByEpisode(
                          item._embedded?.episode
                        )}#${item._embedded?.episode?.id}`}
                        className="flex items-center gap-5 text-neutral-600 text-sm"
                      >
